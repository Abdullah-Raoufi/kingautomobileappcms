...
const navigation = [
  ...
  { name: 'Cars', href: route('cars'), current: false },
  { name: 'Bits', href: route('bits'), current: false },
      ...
      <div class="mx-auto max-w-7xl px-2 sm:px-4 lg:divide-y lg:divide-gray-200 lg:px-8">
              ...
              <transition enter-active-class="transition ease-out duration-100"
                ...
                <MenuItems
                  ...
                  <a :href="route('profile.show')"
                    :class="[active ? 'bg-gray-100' : '', 'block py-2 px-4 text-sm text-gray-700']">Your Profile</a>

                  <form @submit.prevent="logout">
                    <Button type="submit"
                      :class="[active ? 'bg-gray-100' : '', 'block py-2 px-4 text-sm text-gray-700']">Sign out</Button>
        ...
        <nav class="hidden lg:flex lg:space-x-8 lg:py-2" aria-label="Global">
          <Link v-for="item in navigation" :key="item.name" :href="item.href"
            :class="[item.current ? 'bg-gray-100 text-gray-900' : 'text-gray-900 hover:bg-gray-50 hover:text-gray-900', 'rounded-md py-2 px-3 inline-flex items-center text-sm font-medium']"
